...
const uploadCorrigesTypeDir = 'uploads/corriges-type';
...
if (!fs.existsSync(uploadCorrigesTypeDir)) {
  fs.mkdirSync(uploadCorrigesTypeDir, { recursive: true });
}

// Configuration du stockage des fichiers examens
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, uploadDir); // Dossier où les fichiers examens seront stockés
  },
...
// Configuration du stockage des fichiers corrigés types
const storageCorrigesType = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, uploadCorrigesTypeDir); // Dossier où les fichiers corrigés types seront stockés
  },
  filename: function (req, file, cb) {
    // Générer un nom de fichier unique avec la date et l'extension d'origine
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1e9);
...
// Configuration de multer pour les fichiers examens
const upload = multer({
  storage: storage,
...
// Configuration de multer pour les fichiers de soumissions
...
// Configuration de multer pour les fichiers corrigés types
const uploadCorrigeType = multer({
  storage: storageCorrigesType,
  fileFilter: fileFilter,
  limits: { fileSize: 10 * 1024 * 1024 }, // Limite de 10 Mo
});
...
module.exports = { upload, uploadSubmission, uploadCorrigeType, handleUploadErrors };