...
const TeacherStatistics = () => {
  ...
  useEffect(() => {
    const fetchStatistics = async () => {
      try {
        const response = await axios.get("/api/submissions/statistics/1"); // Remplace 1 par l'ID de l'examen
        const { moyenne, distribution, tauxReussite } = response.data; // Supposons que l'API renvoie un taux de réussite
        setAverage(moyenne);
        setGradesDistribution(Object.values(distribution));
        setSuccessRate(tauxReussite); // Mettre à jour successRate
      } catch (error) {
