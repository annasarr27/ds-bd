...
const Dashboard = () => {
  ...
  const [question, setQuestion] = useState(""); // État pour la question du chatbot
  const [response, setResponse] = useState(""); // État pour la réponse du chatbot
  ...
  useEffect(() => {
    const fetchExams = async () => {
      try {
        ...
        setExams(response.data); // Mettre à jour la liste des examens
      } catch (error) {
  ...
  const handleFileUpload = (event) => {
    ...
    console.log("Fichier sélectionné :", file); // Log du fichier sélectionné
  };

  // Gérer la sélection de l'examen
  const handleExamChange = (event) => {
    setExamId(event.target.value);
    console.log("Examen sélectionné :", event.target.value); // Log de l'examen sélectionné
  };
  ...
  const handleSubmit = async () => {
    ...
    console.log("Données envoyées :", { exam_id: examId, fichier_submission: selectedFile }); // Log des données envoyées
    ...
    try {
      ...
      console.log("Réponse du backend :", response); // Log de la réponse du backend
      ...
      console.error("Détails de l'erreur :", error.response); // Log des détails de l'erreur
  ...
  const handleChatbotSubmit = async () => {
    ...
    try {
      ...
      const chatbotResponse = response.data.reponse; // Supposons que le backend renvoie la réponse dans `reponse`
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 4, mb: 4 }}>
      ...
      <Paper elevation={3} sx={{ p: 3, borderRadius: 2, mb: 4 }}>
        ...
        {selectedFile && ( // Afficher le nom du fichier sélectionné
